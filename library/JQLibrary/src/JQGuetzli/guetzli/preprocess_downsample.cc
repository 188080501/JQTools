...
std::vector<float> Convolve2X(const std::vector<float>& image, int w, int h,
  ...
  for (int i = 0; i < (int)image.size(); i++) {
    ...
    temp[i] = static_cast< float >( v * mul );
  }
  auto result = temp;
  for (int i = 0; i < (int)temp.size(); i++) {
    ...
    result[i] = static_cast< float >( v * mul );
  }
  return result;
}
...
std::vector<float> Sharpen(const std::vector<float>& image, int w, int h,
  ...
  std::vector<float> result =
      Convolve2X(image, w, h, kernel.data(), (int)kernel.size(), mul);
...
std::vector<float> Blur(const std::vector<float>& image, int w, int h) {
    ...
    return Convolve2X(image, w, h, kernel.data(), (int)kernel.size(), mul);
}
...
inline float LinearToGamma(float x) {
  return 255.0f * std::pow(x, 1.0f / 2.2f);
}
