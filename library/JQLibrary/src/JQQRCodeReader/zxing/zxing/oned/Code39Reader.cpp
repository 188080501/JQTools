...
Ref<Result> Code39Reader::decodeRow(int rowNumber, Ref<BitArray> row) {
  ...
  { // Arrays.fill(counters, 0);
    int size = static_cast< int >( theCounters.size() );
  ...
  do {
    ...
    lastStart = nextStart;
    for (int i = 0, end=static_cast< int >( theCounters.size() ); i < end; i++) {
  ...
  for (int i = 0, e = static_cast< int >( theCounters.size() ); i < e; i++) {
  ...
  if (usingCheckDigit) {
    ...
    int total = 0;
    for (int i = 0; i < max; i++) {
      total += static_cast< int >( checkdigit_string.find_first_of(decodeRowResult[i], 0) );
    }
